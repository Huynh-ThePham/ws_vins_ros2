...
public:
	PoseGraph();
	~PoseGraph();
	void setPublishCallback(pht::PoseGraphPublishCallback cb);
	...
	pht::PathTrajectory path[10];
	pht::PathTrajectory base_path;
	pht::PoseGraphDebugGraph debug_graph;
	void savePoseGraph();
	void loadPoseGraph();
	void publish();
	Vector3d t_drift;
...
private:
	...
	void optimize6DoF();
	void updatePath();
	pht::PoseStamped makePoseStamped(double time_sec, const Vector3d &P, const Quaterniond &Q) const;
	...
	pht::PoseGraphPublishCallback publish_cb_;
};
