...
Chính sách chỉ dùng tín hiệu online---tỷ lệ burst, EMA semantic mạnh, và chồng lấp hai chiều semantic--geo trên ứng viên GeoDF thô---không đọc nhãn dataset, ground truth hay metric hold-out.
...
Chúng tôi \emph{không} khẳng định thắng từng ô so với SAD-Sem về ATE trung bình; đóng góp là độ bền phổ động, vệ sinh giao thức, và đánh giá bốn phương pháp tái lập được trên VIODE (\(N=\PaperNViode\) trial/ô) và EuRoC (tổng quát hóa tĩnh, \(N=\PaperNEuroc\) trial/ô).
...
Phạm vi của chúng tôi hẹp và bổ trợ: module front-end stereo-inertial gắn vào VINS-Fusion với ràng buộc realtime rõ (mặt nạ không chặn), chính sách online không nhãn, trọng số backend tùy chọn, và vệ sinh giao thức (không rò nhãn vào ước lượng online).
...
\subsection{Chồng lấp hai chiều trên ứng viên GeoDF thô}
\label{sec:overlap}
...
Sem-GeoDF dùng pool ứng viên GeoDF thô \(G\) (fallback confirmed nếu rỗng) và tập semantic-raw \(S\) (Hình~\ref{fig:overlap}):
\begin{equation}
o=\max\!\left(\frac{|G\cap S|}{|G|},\frac{|S\cap G|}{|S|}\right),
\end{equation}
rồi làm mượt EMA với hệ số \(\texttt{sem\_policy\_overlap\_ema}\).
Bảo vệ số ứng viên đạt nếu \(|G|\) hoặc \(|S|\) đạt \(\texttt{sem\_policy\_min\_geo\_candidates}\).
Điều này giữ trigger chồng lấp sống khi hai chuyên gia kích hoạt trên tập track kích thước khác nhau.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{overlap_bidirectional.pdf}
\caption{Chồng lấp hai chiều trên pool GeoDF thô. Lấy max hai tỷ lệ có hướng giữ trigger đồng thuận khi tập semantic và hình học khác kích thước.}
\label{fig:overlap}
\end{figure}
...
Leo thang dùng ba vị từ online và bộ đếm hold \(H\) khung:
\begin{align}
...
Bộ đếm hold chống rung sau trigger burst hoặc overlap.
\begin{itemize}
...
\caption{Luồng trọng số dư thích nghi cho track sống sót ngân sách loại bỏ chung. Rủi ro semantic/hình học online tạo mục tiêu OR xác suất \(\tilde{w}_i\), phục hồi theo id, rồi áp \(\sqrt{w_i}\) trong Ceres.}
\label{fig:weight}
\end{figure}
...
Hợp OR xác suất cho trọng số mục tiêu
\begin{equation}
\tilde{w}_i = \mathrm{clip}\!\left(1-(1{-}\rho^{\mathrm{sem}}_i)(1{-}\rho^{\mathrm{geo}}_i)(1{-}\rho^{\mathrm{agr}}_i),\; w_{\min},\;1\right),
\end{equation}
rồi cắt \(\tilde{w}_i\) bởi \(w_{\mathrm{sem}}\), \(w_{\mathrm{geo}}\), hoặc \(w_{\mathrm{agr}}\) khi cờ xác nhận tương ứng bật.
...
\end{cases}
\end{equation}
Cột audit trong \(\texttt{sem\_geodf\_stats.csv}\) ghi \(\texttt{weighted\_tracks}\), \(\texttt{mean\_backend\_weight}\), và chẩn đoán liên quan.
...
\subsection{Độ nhạy và hành vi realtime}
...
Đồng bộ mặt nạ không chặn giữ VINS realtime: ước lượng không bao giờ chờ YOLO; nó hoặc dùng mặt nạ đủ mới hoặc fallback chỉ-GeoDF.
