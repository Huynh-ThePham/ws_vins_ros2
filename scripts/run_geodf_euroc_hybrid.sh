#!/usr/bin/env bash
# Run GeoDF-Hybrid (Paper #2) on EuRoC static-safety sequences.
#
# Usage: ./scripts/run_geodf_euroc_hybrid.sh [N] [SEQUENCES...]
set -eo pipefail
...
source_ros2_ws "$WS"
MODE="$(geodf_method_to_mode hybrid)"
...
for SEQ in "$@"; do
    ...
    for i in $(seq 1 "$N"); do
        out="${OUT_ROOT}/${SEQ}_hybrid/trial_${i}"
        if [ "${FORCE:-0}" != "1" ] && [ -f "${out}/eval/metrics.json" ]; then
            echo "[have] ${SEQ}_hybrid trial ${i}"
            continue
        fi
        mkdir -p "$out"
        ...
        echo "=== ${SEQ}_hybrid trial ${i}/${N} start=${START}s ==="
        ...
        python3 "${WS}/scripts/evaluate_trajectory.py" \
            ...
            --run-name "${SEQ}_hybrid_t${i}" || echo "[warn] eval failed ${SEQ} t${i}"
    done
done
echo "[euroc hybrid] done -> ${OUT_ROOT}"
