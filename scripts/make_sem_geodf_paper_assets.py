...
def collect(root: Path):
    ...
    for rec in iter_run_records(root):
        if not rec.qc_ok or rec.ate_rmse_m is None or rec.oracle_ablation:
            continue
        # Drop obviously diverged runs
        if rec.ate_rmse_m > 50.0:
            continue
        by[(rec.scene, rec.method)].append(rec.ate_rmse_m)
    return by


def ate(by, scene, method):
    vals = by.get((scene, method), [])
    if not vals:
        return None, None, 0
