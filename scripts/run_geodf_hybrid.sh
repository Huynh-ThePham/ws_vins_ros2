#!/usr/bin/env bash
# Run GeoDF-Hybrid (Paper #2) on VIODE conditions.
#
# Usage: ./scripts/run_geodf_hybrid.sh "[LEVELS]" "[ENVS]" [N]
set -eo pipefail
...
for env in $ENVS; do
    for level in $LEVELS; do
        ...
        for i in $(seq 1 "$N"); do
            out="${OUT_ROOT}/${env}_${level}_hybrid/trial_${i}"
            if [ "${FORCE:-0}" != "1" ] && [ -f "${out}/eval/metrics.json" ]; then
                echo "[have] ${env}_${level}_hybrid trial ${i}"
                continue
            fi
            mkdir -p "$out"
            ...
            echo "=== ${env}_${level}_hybrid trial ${i}/${N} (mode=${mode}) ==="
            ...
            python3 "${WS}/scripts/evaluate_trajectory.py" \
                ...
                --run-name "${env}_${level}_hybrid_t${i}" || echo "[warn] eval failed ${env}/${level} t${i}"
        done
    done
done
