#!/usr/bin/env python3
"""Aggregate per-trial resource.json into a baseline-vs-weighted resource table.

Reports mean peak RSS, mean CPU%, and thread count per method across all trials,
so the paper can state end-to-end computational cost and show that GeoDF-Weighted
...
def main() -> None:
    ...
    for root in args.roots:
        ...
        bundle[label] = {}
        for method in ("baseline", "weighted"):
            r = collect(rp, method)
