...
class IntegrationBase
{
  public:
...

    {
        ...
        noise.block<3, 3>(0, 0) =  (vinsConfig().acc_n * vinsConfig().acc_n) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(3, 3) =  (vinsConfig().gyr_n * vinsConfig().gyr_n) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(6, 6) =  (vinsConfig().acc_n * vinsConfig().acc_n) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(9, 9) =  (vinsConfig().gyr_n * vinsConfig().gyr_n) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(12, 12) =  (vinsConfig().acc_w * vinsConfig().acc_w) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(15, 15) =  (vinsConfig().gyr_w * vinsConfig().gyr_w) * Eigen::Matrix3d::Identity();
    }
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        residuals.block<3, 1>(O_P, 0) = Qi.inverse() * (0.5 * vinsConfig().g * sum_dt * sum_dt + Pj - Pi - Vi * sum_dt) - corrected_delta_p;
        residuals.block<3, 1>(O_R, 0) = 2 * (corrected_delta_q.inverse() * (Qi.inverse() * Qj)).vec();
        residuals.block<3, 1>(O_V, 0) = Qi.inverse() * (vinsConfig().g * sum_dt + Vj - Vi) - corrected_delta_v;
...
/*
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        if(update_jacobian)
        {
            ...
            noise.block<3, 3>(0, 0) =  (vinsConfig().acc_n * vinsConfig().acc_n) * Eigen::Matrix3d::Identity();
            noise.block<3, 3>(3, 3) =  (vinsConfig().gyr_n * vinsConfig().gyr_n) * Eigen::Matrix3d::Identity();
            noise.block<3, 3>(6, 6) =  (vinsConfig().acc_w * vinsConfig().acc_w) * Eigen::Matrix3d::Identity();
            noise.block<3, 3>(9, 9) =  (vinsConfig().gyr_w * vinsConfig().gyr_w) * Eigen::Matrix3d::Identity();
