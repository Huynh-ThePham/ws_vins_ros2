#!/usr/bin/env bash
# Run the GeoDF-Inertial (Paper #2) method on VIODE conditions using the prepared
# ROS 2 bags + cached ground-truth CSVs (the raw .bag files may be unmounted).
# Reuses existing trials unless FORCE=1.
#
# Usage: ./scripts/run_geodf_inertial.sh "[LEVELS]" "[ENVS]" [N]
...
for env in $ENVS; do
    for level in $LEVELS; do
        ...
        for i in $(seq 1 "$N"); do
            out="${OUT_ROOT}/${env}_${level}_inertial/trial_${i}"
            if [ "${FORCE:-0}" != "1" ] && [ -f "${out}/eval/metrics.json" ]; then
                echo "[have] ${env}_${level}_inertial trial ${i}"
                continue
            fi
            mkdir -p "$out"
            ...
            echo "=== ${env}_${level}_inertial trial ${i}/${N} (mode=${mode}) ==="
            ...
            python3 "${WS}/scripts/evaluate_trajectory.py" \
                ...
                --run-name "${env}_${level}_inertial_t${i}" || echo "[warn] eval failed ${env}_${level} t${i}"
        done
    done
done
echo "[inertial] done -> ${OUT_ROOT}"
