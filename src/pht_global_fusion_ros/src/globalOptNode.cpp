...
void publish_car_model(double t, Eigen::Vector3d t_w_car, Eigen::Quaterniond q_w_car)
{
    ...
    car_mesh.mesh_resource = "package://pht_global_fusion_ros/models/car.dae";
...
void vio_callback(const nav_msgs::msg::Odometry::SharedPtr pose_msg)
{
    ...
    {
        sensor_msgs::msg::NavSatFix::SharedPtr GPS_msg = gpsQueue.front();
        double gps_t = rclcpp::Time(GPS_msg->header.stamp).seconds();
        printf("vio t: %f, gps t: %f \n", t, gps_t);
        // 10ms sync tolerance
        if(gps_t >= t - 0.01 && gps_t <= t + 0.01)
        {
            //printf("receive GPS with timestamp %f\n", rclcpp::Time(GPS_msg->header.stamp).seconds());
    ...
    std::ofstream foutC("/tmp/vio_global.csv", ios::app);
    foutC.setf(ios::fixed, ios::floatfield);
    foutC.precision(0);
    foutC << rclcpp::Time(pose_msg->header.stamp).seconds() * 1e9 << ",";
    foutC.precision(5);
    foutC << global_t.x() << ","
            << global_t.y() << ","
            << global_t.z() << ","
