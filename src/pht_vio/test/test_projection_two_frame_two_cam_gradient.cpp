// Gradient test for ProjectionTwoFrameTwoCamFactor at w in {1.0, 0.75, 0.25}.
...
int main()
{
    ...
    const std::vector<const double *> values{pose_i, pose_j, ex_pose0, ex_pose1, inv_dep, td};

    for (double weight : gradient_check::weights())
    {
        const std::string label =
            "ProjectionTwoFrameTwoCamFactor.Gradient w=" + std::to_string(weight);
        TEST_CASE(label.c_str());
        const ProjectionTwoFrameTwoCamFactor factor(pts_i, pts_j, velocity_i, velocity_j,
                                                    0.0, 0.011, weight);
