#!/usr/bin/env python3
"""Aggregate geo_ms / frame_active from all weighted repeat trials (Table 5)."""
...
def aggregate(root: Path, pattern: str = "*_weighted/trial_*/geo_df_stats.csv") -> dict:
    geo_ms: list[float] = []
    active = total = 0
    trials = 0
...
def main() -> None:
    ...
    lines = [
        "# GeoDF-Weighted runtime (aggregated from repeat trials)\n",
        "| Metric | Value |",
        "|---|---:|",
        ...
        f"| Logged frames | {m['frames']:,} ({m['trials']} weighted trials) |",
        "",
    ]
