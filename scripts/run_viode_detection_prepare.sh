#!/usr/bin/env bash
# One-time GeoDF-Weighted runs (12 env×level) for feature-level detection eval.
#
# Uses the weighted config (geodf_dump_features: 1) and evaluates dynamic-feature
# detection with prediction=weight (weight < threshold marks a dynamic candidate).
#
# Prerequisites: run_viode_n5_prepare.sh
# Usage: ./scripts/run_viode_detection_prepare.sh
# Env: VIODE_ROOT, FORCE=1, WEIGHT_THRESHOLD=0.999
set -eo pipefail

WS="$(cd "$(dirname "$0")/.." && pwd)"
export VIODE_ROOT="${VIODE_ROOT:-/media/theph/Data1/Research/Datasets/Viode}"
export FORCE="${FORCE:-0}"
WEIGHT_THRESHOLD="${WEIGHT_THRESHOLD:-0.999}"
...
for env in "${VIODE_ENVS[@]}"; do
    for level in $LEVELS; do
        ...
        if [ "$FORCE" != "1" ] && [ -f "${out}/geo_df_features.csv" ]; then
            ...
            run_cfg="${VIODE_CFG}/viode_${mode}_config_det_${env}_${level}.yaml"
            mkdir -p "$out"
...
for p in sorted(glob.glob("results/viode_detection/*_weighted/detection_eval.json")):
    ...
    env, level = name.rsplit("_", 1)
    rows.append((env, level, d))
lines = ["# VIODE detection eval (GeoDF-Weighted, weight threshold)", "",
