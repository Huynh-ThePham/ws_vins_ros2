...
{
    // Numeric residuals come from Evaluate() after perturbation — no duplicated
    // residual math (ProjectionFactor has no sqrt_weight, but keep the same path).
    const std::vector<projection_factor_check::Block> blocks{
        projection_factor_check::pose(), projection_factor_check::pose(),
        projection_factor_check::pose(), projection_factor_check::scalar()};
