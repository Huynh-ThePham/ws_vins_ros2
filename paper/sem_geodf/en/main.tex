...
\caption{Adaptive residual-weight path for tracks that survive the shared reject budget. Online semantic/geometric risks form a probabilistic-OR target \(\tilde{w}_i\), recovered per id, then applied as \(\sqrt{w_i}\) in Ceres.}
\label{fig:weight}
\end{figure}
...
A probabilistic-OR combines them into a target weight
\begin{equation}
\tilde{w}_i = \mathrm{clip}\!\left(1-(1{-}\rho^{\mathrm{sem}}_i)(1{-}\rho^{\mathrm{geo}}_i)(1{-}\rho^{\mathrm{agr}}_i),\; w_{\min},\;1\right),
\end{equation}
then caps \(\tilde{w}_i\) by \(w_{\mathrm{sem}}\), \(w_{\mathrm{geo}}\), or \(w_{\mathrm{agr}}\) when the corresponding confirmation flags fire.
...
\end{cases}
\end{equation}
Audit columns in \(\texttt{sem\_geodf\_stats.csv}\) record \(\texttt{weighted\_tracks}\), \(\texttt{mean\_backend\_weight}\), and related diagnostics.
