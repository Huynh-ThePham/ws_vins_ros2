#!/usr/bin/env python3
"""Statistical significance of GeoDF-Adaptive vs baseline (top-tier evidence).

For every condition it loads the per-trial ATE (and RPE) samples for baseline
and adaptive and reports, per condition and in aggregate:
...
def build_rows(cells: list[tuple[str, str]], root: str, key: str) -> tuple[list, dict]:
    ...
    for label, seqbase in cells:
        ...
        a = load(f"{root}/{seqbase}_adaptive", key)
        c = compare(b, a)
        bundle[label] = c
