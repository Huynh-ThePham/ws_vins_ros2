...
// TF broadcaster - initialized in registerPub
...
void printStatistics(const Estimator &estimator, double t)
{
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("vins"), "position: %f %f %f", estimator.Ps[WINDOW_SIZE].x(), estimator.Ps[WINDOW_SIZE].y(), estimator.Ps[WINDOW_SIZE].z());
    RCLCPP_DEBUG(rclcpp::get_logger("vins"), "orientation: %f %f %f", estimator.Vs[WINDOW_SIZE].x(), estimator.Vs[WINDOW_SIZE].y(), estimator.Vs[WINDOW_SIZE].z());
    if (vinsConfig().estimate_extrinsic)
    {
        cv::FileStorage fs(vinsConfig().ex_calib_result_path, cv::FileStorage::WRITE);
    ...
    sum_of_time += t;
    sum_of_calculation++;
    RCLCPP_DEBUG(rclcpp::get_logger("vins"), "vo solver costs: %f ms", t);
    RCLCPP_DEBUG(rclcpp::get_logger("vins"), "average of time %f ms", sum_of_time / sum_of_calculation);

    sum_of_path += (estimator.Ps[WINDOW_SIZE] - last_path).norm();
    last_path = estimator.Ps[WINDOW_SIZE];
    RCLCPP_DEBUG(rclcpp::get_logger("vins"), "sum of path %f", sum_of_path);
    if (vinsConfig().estimate_td)
        RCLCPP_INFO(rclcpp::get_logger("vins"), "td %f", estimator.td);
}
...
void pubOdometry(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        ofstream foutC(vinsConfig().vins_result_path, ios::app);
        foutC.setf(ios::fixed, ios::floatfield);
        foutC.precision(0);
        double stamp_sec = header.stamp.sec + header.stamp.nanosec * 1e-9;
        foutC << stamp_sec * 1e9 << ",";
        foutC.precision(5);
        ...
        printf("time: %f, t: %f %f %f q: %f %f %f %f \n", stamp_sec, tmp_T.x(), tmp_T.y(), tmp_T.z(),
...
void pubTF(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    ...
    cam_transform.header = header;
    cam_transform.header.frame_id = "body";
    cam_transform.child_frame_id = "camera";
    cam_transform.transform.translation.x = estimator.tic[0].x();
    cam_transform.transform.translation.y = estimator.tic[0].y();
    cam_transform.transform.translation.z = estimator.tic[0].z();
    Quaterniond cam_q(estimator.ric[0]);
    cam_transform.transform.rotation.w = cam_q.w();
    cam_transform.transform.rotation.x = cam_q.x();
    cam_transform.transform.rotation.y = cam_q.y();
    cam_transform.transform.rotation.z = cam_q.z();
    tf_broadcaster_->sendTransform(cam_transform);
