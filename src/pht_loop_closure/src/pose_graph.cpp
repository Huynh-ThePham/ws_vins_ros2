...
pht::PoseStamped PoseGraph::makePoseStamped(double time_sec, const Vector3d &P, const Quaterniond &Q) const
{
    pht::PoseStamped ps;
    ps.time_sec = time_sec;
    ps.position = P + Vector3d(loopClosureConfig().visualization_shift_x,
                              loopClosureConfig().visualization_shift_y, 0);
    ps.orientation = Q;
    return ps;
}

void PoseGraph::setPublishCallback(pht::PoseGraphPublishCallback cb)
{
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    Quaterniond Q{R};
    auto pose_stamped = makePoseStamped(cur_kf->time_stamp, P, Q);
    path[sequence_cnt].frame_id = "world";
    path[sequence_cnt].poses.push_back(pose_stamped);

    if (SAVE_LOOP_PATH)
    {
        ofstream loop_path_file(loopClosureConfig().vins_result_path, ios::app);
    ...
    if (SHOW_S_EDGE)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if((*rit)->sequence == cur_kf->sequence)
            {
                ...
                debug_graph.add_edge(P, conncected_P);
            }
            rit++;
        }
    }
    if (SHOW_L_EDGE)
    {
        if (cur_kf->has_loop)
        {
            ...
            if(cur_kf->sequence > 0)
            {
                ...
                debug_graph.add_loopedge(P0, connected_P + Vector3d(loopClosureConfig().visualization_shift_x, loopClosureConfig().visualization_shift_y, 0));
            }
            
        }
    }
    //debug_graph.add_pose(P + Vector3d(loopClosureConfig().visualization_shift_x, loopClosureConfig().visualization_shift_y, 0), Q);
...
void PoseGraph::loadKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    if (cur_kf->has_loop)
    {
        ...
        Matrix3d connected_R;
        connected_KF->getPose(connected_P,  connected_R);
        debug_graph.add_loopedge(P, connected_P, SHIFT);
    }
    */
...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    if (loopClosureConfig().debug_image)
    {
    ...
    if (ret.size() >= 1 &&ret[0].Score > 0.05)
        for (unsigned int i = 1; i < ret.size(); i++)
        {
            ...
            {          
                find_loop = true;
                int tmp_index = ret[i].Id;
                if (loopClosureConfig().debug_image && 0)
                {
                    auto it = image_pool.find(tmp_index);
                    cv::Mat tmp_image = (it->second).clone();
...
void PoseGraph::optimize4DoF()
{
    while (!shutdown_.load())
    {
        ...
        for (int i = 0; i < 40 && !shutdown_.load(); ++i)
...
void PoseGraph::updatePath()
{
    ...
    if (SAVE_LOOP_PATH)
    {
        ofstream loop_path_file_tmp(loopClosureConfig().vins_result_path, ios::out);
...
//        printf("path p: %f, %f, %f\n",  P.x(),  P.z(),  P.y() );
        ...
        auto pose_stamped = makePoseStamped((*it)->time_stamp, P, Q);
        if((*it)->sequence == 0)
        {
            base_path.frame_id = "world";
            base_path.poses.push_back(pose_stamped);
        ...
        {
            path[(*it)->sequence].frame_id = "world";
        ...
        if (SHOW_L_EDGE)
        {
            if ((*it)->has_loop && (*it)->sequence == sequence_cnt)
            {
                ...
                if((*it)->sequence > 0)
                {
                    debug_graph.add_loopedge(P, connected_P + Vector3d(loopClosureConfig().visualization_shift_x, loopClosureConfig().visualization_shift_y, 0));
                }
            }
        }
...
void PoseGraph::savePoseGraph()
{
    ...
    TicToc tmp_t;
    FILE *pFile;
    printf("pose graph path: %s\n",loopClosureConfig().pose_graph_save_path.c_str());
    printf("pose graph saving... \n");
    string file_path = loopClosureConfig().pose_graph_save_path + "pose_graph.txt";
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        ...
        if (loopClosureConfig().debug_image)
        {
            image_path = loopClosureConfig().pose_graph_save_path + to_string((*it)->index) + "_image.png";
        ...
        brief_path = loopClosureConfig().pose_graph_save_path + to_string((*it)->index) + "_briefdes.dat";
        std::ofstream brief_file(brief_path, std::ios::binary);
        keypoints_path = loopClosureConfig().pose_graph_save_path + to_string((*it)->index) + "_keypoints.txt";
        FILE *keypoints_file;
...
void PoseGraph::loadPoseGraph()
{
                                    ...
                                    &keypoints_num) != EOF) 
    {
        ...
        */
        ...
        if (loopClosureConfig().debug_image)
        {
            image_path = loopClosureConfig().pose_graph_save_path + to_string(index) + "_image.png";
        ...
        string brief_path = loopClosureConfig().pose_graph_save_path + to_string(index) + "_briefdes.dat";
        std::ifstream brief_file(brief_path, std::ios::binary);
        string keypoints_path = loopClosureConfig().pose_graph_save_path + to_string(index) + "_keypoints.txt";
        FILE *keypoints_file;
...
void PoseGraph::publish()
{
    ...
    pht::PoseGraphSnapshot snap;
    snap.base_path = base_path;
    for (int i = 0; i < 10; ++i)
        snap.paths[i] = path[i];
    snap.sequence_cnt = sequence_cnt;
    snap.base_sequence = base_sequence;
    snap.debug = debug_graph;
    publish_cb_(snap);
}
