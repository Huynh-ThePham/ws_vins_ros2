#!/usr/bin/env bash
# VIODE-only N-repeat benchmark (baseline + weighted on all 12 conditions).
#
...
echo "[viode-n5] FORCE=$FORCE  METHODS=$METHODS  total_runs=$(( 3 * 4 * 2 * N ))"

python3 "${WS}/scripts/capture_provenance.py" \
    --study-dir "${WS}/results/viode_repeat" --study "viode_n5_weighted" \
...
for env in "${VIODE_ENVS[@]}"; do
    ...
    bash "${WS}/scripts/run_geodf_weighted.sh" "$ALL_LEVELS" "$env" "$N"
done
unset SKIP_SUMMARY
